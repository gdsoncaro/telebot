...
class TelegramGPTBot:
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
      ...
      pdf_status = f"📚 PDFs Cargados: {len(self.doc_store.pdf_files)}" if self.doc_store.pdf_files else "⚠️ No PDFs loaded"
      
      welcome_message = f"""
      👋 Hola! Podes preguntarme lo que quieras acerca de las fichas de catedra del 2do parcial de Antropologia.
      
      {pdf_status}
      Mensajes que te quedan: {remaining_messages}
      
      Comandos:
      /start - Mostrar este mensaje
      /clear - Borrar tu historial de conversacion
      /quota - Ver cuantos mensajes te quedan
      /pdfs - Ver los PDFs cargados
      
      Podrias preguntarme: Enumera y explica los conceptos centrales presentados en [nombre de ficha de catedra]!
      """
