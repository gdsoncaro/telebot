...
class AudioProcessor:
    def __init__(self, client: openai.OpenAI):
        self.client = client
        self.temp_dir = Path(tempfile.gettempdir()) / "telegram_audio"
        self.temp_dir.mkdir(exist_ok=True)
    
    async def process_voice_message(self, file_bytes: bytes) -> str:
        """Convert voice message to text using Whisper API."""
        try:
            # Create a temporary file for the audio
            temp_file = self.temp_dir / f"voice_{datetime.now().timestamp()}.oga"
            
            # Save the audio bytes to the temp file
...
class TelegramGPTBot:
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        You can send me text messages or voice messages!
    ...
    async def help_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        4. Maintaining context of our conversation
    ...
    async def handle_voice_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            transcribed_text = await self.audio_processor.process_voice_message(bytes(voice_bytes))
            
            # Log the transcription
            logger.info(f"Transcribed voice message from user {user_id}: {transcribed_text[:100]}...")
            
            # Send transcription to user
            await update.message.reply_text(
                f"🎤 Escuché que dijiste: \"{transcribed_text}\"\n\nPensando tu respuesta..."
            )
            
            # Instead of modifying the update.message.text, we'll process the transcribed text directly
            await context.bot.send_chat_action(
                chat_id=update.effective_chat.id,
                action='typing'
            )

            relevant_context = self.doc_store.query_documents(transcribed_text)
            ...
            self.rate_limiter.add_message(user_id)
            self.conversation_manager.add_message(user_id, "user", transcribed_text)
            self.conversation_manager.add_message(user_id, "assistant", ai_response)
            ...
            error_message = f"Sorry, I couldn't process your voice message: {str(e)}"
            await update.message.reply_text(error_message)
            logger.error(f"Error handling voice message from user {user_id}: {str(e)}")
    ...
    def run(self) -> None:
        ...
        try:
            ...
            # Add handlers for both text and voice messages
            ...
            logger.info("Bot started with voice message support")
